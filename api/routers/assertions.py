...
def create_undirected_assertion(obj: dict):
    rid = RID.from_string(rid="und_asrt:" + nanoid.generate())
...
def create_directed_assertion(obj: dict):
    rid = RID.from_string(rid="dir_asrt:" + nanoid.generate())
    return database.create_directed_assertion(rid, obj)

@router.post("/{forked_rid}/fork")
def fork_assertion(forked_rid: str):
    rid = RID.from_string(rid=forked_rid)
    new_rid = rid.means(nanoid.generate())
    return database.fork_assertion(rid, new_rid)
...
def update_assertion(rid: str, obj: dict):
    return database.update_assertion(RID.from_string(rid=rid), obj)
...
def update_assertion_definition(rid: str, obj: dict):
    return database.update_assertion_definition(RID.from_string(rid=rid), obj)
...
def update_undirected_assertion_members(rid: str, obj: dict):
    return database.update_undirected_assertion_members(RID.from_string(rid=rid), obj)
...
def update_directed_assertion_members(rid: str, obj: dict):
    return database.update_directed_assertion_members(RID.from_string(rid=rid), obj)

# The following three endpoints are identical but cover three different paths
@router.delete("/{rid}")
def delete_assertion(rid: str):
    return database.delete_assertion(RID.from_string(rid=rid))
